...
def Model_Accommodate(nnL,nnR,ML,MR):
    outL = ((nnL*abs(LD.last_nnL-nnL))+(ML*abs(LD.last_ML-ML)))/(abs(LD.last_nnL-nnL)+abs(LD.last_ML-ML))
    outR = ((nnR*abs(LD.last_nnR-nnR))+(MR*abs(LD.last_MR-MR)))/(abs(LD.last_nnR-nnR)+abs(LD.last_MR-MR))
    LD.last_nnL = nnL
    LD.last_nnR = nnR
    LD.last_ML = ML
    LD.last_MR = MR
