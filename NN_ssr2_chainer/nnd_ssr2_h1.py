...
def Model_Accommodate(nnL,nnR,ML,MR):
    outL = ((nnL*abs(last_nnL-nnL))+(ML*abs(last_ML-ML)))/(abs(last_nnL-nnL) + abs(last_ML-ML))
    outR = ((nnR*abs(last_nnR-nnR))+(MR*abs(last_MR-MR)))/(abs(last_nnR-nnR) + abs(last_MR-MR))
    last_nnL = nnL
    last_nnR = nnR
    last_ML = ML
    last_MR = MR
